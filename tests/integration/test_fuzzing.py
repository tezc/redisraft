...
def test_fuzzing_with_large_snapshots(cluster, workload):
    ...
    for i in range(40):
        ...
        if op == Operation.ADD_NODE:
            # Remove a random node and add a new one instead.
            cluster.remove_node(cluster.random_node_id())
            cluster.add_node()
            ...
            # Restart a random follower node and sleep a bit before starting
            # it back. So, node will lag behind and entries and/or snapshot
            # will be delivered.
            n = cluster.follower_node()
            n.terminate()
            time.sleep(10)
            ...
            # Leader transfer can fail, e.g. destination node won't catch up
